// ==UserScript==
// @name         BMS Markdown URL button
// @namespace    https://github.com/outsideopen/userscripts
// @version      0.2
// @description  adds a "Markdown URL" button to the buttons on a ticket (usefule for non-slack)
...
(function() {
	...
	{
		return parseQueryVariable(window.location.search.substring(1), variable);
	}
	...
	function copy_to_clipboard()
	{
		...
		clip.setValue("[" + client + ' :: ' + title + "](" + newURL + "?ID=" + parseAspNetFormActionVariable('ID') + ")");
	...
	jQuery('#ctl00_phMenu_pnlGroupActionButtons').append('<button id="custom-button-markdown" class="btn btn-sm btn-primary">Markdown URL</a>');
	jQuery('#custom-button-markdown').on('click', function(e) {
		e.preventDefault();
		copy_to_clipboard();
	});
